...
fn macos_release_tarball_ships_the_launchd_agent_plist() {
    ...
    if !release.contains("runs-on: macos-") {
        assert!(
            release.contains(
                "This workstation fork intentionally publishes only the native products used"
            ) && release.contains("Linux x86_64 for WSL and Windows x86_64"),
            "a release workflow without macOS must document the fork's intentionally bounded platform scope"
        );
        return;
    }
