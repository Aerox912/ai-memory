...
"#,
    ...
    fn ts_spool_entries_are_readable_by_the_cli_drain() {
        ...
        let json = format!(
            r#"{{"url":"http://127.0.0.1:49374/hook?event=stop&agent=open-code&ingest_key=ts18f3","body":"{{\"session_id\":\"s1\"}}","created_ms":{created_ms},"auth_mode":"static","attempts":0}}"#
        );
        ...
        assert_eq!(
            entry.auth_mode,
            crate::commands::hook_spool::AuthMode::Static
        );
